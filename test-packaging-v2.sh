...
package-and-install () {
  ...
  themeName="--theme-name @thepolicylab-projectportals/gatsby-theme-project-portal"

  # Specify where the pack file is stored
  artifactDir="--artifact-dir $(pwd)/artifacts"
  ...
  zparseopts -D -E -K - \
    ...
    n:=themeName -theme-name:=themeName \
    a:=artifactDir -artifact-dir:=artifactDir \
    m:=packageMethod -package-method:=packageMethod \
    i:=packageManager -package-manager:=packageManager \
    s=serve -serve=serve \
    p:=publishTag -publish-tag:=publishTag
